...
export default function WorkoutsScreen() {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            // Load manual workouts WITH their exercises
            const { supabase } = await import('@/services/supabase');
            const { data: manual } = await supabase.from('user_manual_workouts').select('*').order('created_at', { ascending: false }).limit(10);
            if (manual && manual.length > 0) {
                // Load exercises for each manual workout
                const ids = manual.map(m => m.id);
                const { data: allLogs } = await supabase.from('workout_logs').select('*').in('workout_id', ids);
                const mwWithExercises = manual.map(m => ({
                    ...m,
                    exercises: (allLogs || []).filter(l => l.workout_id === m.id)
                }));
                setManualWorkouts(mwWithExercises);
            } else {
                setManualWorkouts([]);
            }
        } catch (error: any) {
    ...
    const renderWorkoutContent = () => {
        const hasCoachProgram = program && days.length > 0;

        return (
            <View>
                {/* ========= MODE 1: COACH PROGRAM ========= */}
                {hasCoachProgram && (
                    <>
                        {/* Week Day Tabs */}
                        ...
                        {(() => {
                            const sd = days.find(d => d.id === selectedDayId);
                            if (!sd) return (
                                <View style={styles.emptyState}>
                                    <Ionicons name="barbell-outline" size={64} color={Colors.textMuted} />
                                    <Text style={styles.emptyText}>Bir gün seçin.</Text>
                                </View>
                            );
                            return (
                                <>
                                    <Card style={styles.workoutSummaryCard} glow>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                                                ...
                                                <View>
                                                    <Text style={styles.workoutSummaryTitle}>{sd.muscle_group || 'Antrenman'}</Text>
                                                    <Text style={styles.workoutSummarySubtitle}>{sd.exercises?.length || 0} Hareket</Text>
                                                </View>
                                            </View>
                                            <TouchableOpacity style={styles.completeHeaderBtn} onPress={async () => {
                                                try {
                                                    setLoading(true);
                                                    const { completeWorkoutDay } = await import('@/services/api');
                                                    await completeWorkoutDay(sd.id);
                                                    Alert.alert('Tebrikler! 🔥', 'Antrenman tamamlandı!');
                                                    loadData();
                                                } catch (err: any) {
                                                    Alert.alert('Hata', err?.response?.data?.error || 'Hata oluştu.');
                                        ...
                                        {sd.exercises.map((ex: Exercise) => (
                                            <Card key={ex.id} style={styles.exerciseCardNew}>
                                                <View style={styles.exerciseCardRow}>
                                                    ...
                                                    <TouchableOpacity style={styles.howToBtn} onPress={() => Alert.alert('Nasıl Yapılır?', `${ex.name} için video hazırlanıyor! ✨`)}>
                                                        <Ionicons name="videocam-outline" size={20} color={Colors.primary} />
                                                    </TouchableOpacity>
                                                </View>
                                            </Card>
                                        ))}
                                        <TouchableOpacity style={styles.completeWorkoutBtn} onPress={async () => {
                                            try {
                                                setLoading(true);
                                                const { completeWorkoutDay } = await import('@/services/api');
                                                await completeWorkoutDay(sd.id);
                    ...
                    <>
                        ...
                        {manualWorkouts.length > 0 ? (
                            <View style={{ marginTop: 20 }}>
                                <Text style={styles.sectionTitle}>Antrenmanlarım</Text>
                                {manualWorkouts.map(mw => {
                                    const isExpanded = selectedDayId === mw.id;
                                    const dayMatch = mw.workout_name?.match(/^\[(.*?)\]/);
                                    const dayLabel = dayMatch ? dayMatch[1] : '';
                                    const workoutTitle = mw.workout_name?.replace(/^\[.*?\]\s*/, '') || 'Antrenman';

                                    return (
                                        <View key={mw.id} style={{ marginBottom: 10 }}>
                                            <TouchableOpacity
                                                style={[styles.accordionHeader, isExpanded && styles.accordionHeaderActive]}
                                                onPress={() => setSelectedDayId(isExpanded ? null : mw.id)}
                                            >
                                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 }}>
                                                    <View style={[styles.dayCircle, isExpanded && styles.dayCircleActive]}>
                                                        <Text style={[styles.dayCircleText, isExpanded && styles.dayCircleTextActive]}>{dayLabel}</Text>
                                                    </View>
                                                    <View style={{ flex: 1 }}>
                                                        <Text style={styles.muscleText}>{workoutTitle}</Text>
                                                        <Text style={styles.exerciseCountText}>
                                                            {mw.exercises?.length || 0} Hareket • {new Date(mw.created_at).toLocaleDateString('tr-TR')}
                                                        </Text>
                                                    </View>
                                                </View>
                                            ...
                                            {isExpanded && (
                                                <View style={styles.accordionContent}>
                                                    {mw.exercises && mw.exercises.length > 0 ? (
                                                        mw.exercises.map((ex: any, idx: number) => (
                                                            <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 8, borderBottomWidth: idx < mw.exercises.length - 1 ? 1 : 0, borderBottomColor: 'rgba(255,255,255,0.03)' }}>
                                                                <View style={[styles.exerciseIconBox, { width: 40, height: 40 }]}>
                                                                    <Text style={{ color: Colors.primary, fontWeight: '800', fontSize: 14 }}>{idx + 1}</Text>
                                                                </View>
                                                                <View style={{ flex: 1 }}>
                                                                    <Text style={styles.exerciseNameNew}>{ex.exercise_name || ex.name}</Text>
                                                                    <Text style={styles.exerciseMetaNew}>
                                                                        {ex.sets_count || ex.sets} Set • {ex.reps_count || ex.reps} Tekrar{(ex.weight_kg || ex.weight) ? ` • ${ex.weight_kg || ex.weight} kg` : ''}
                                                                    </Text>
                                                                </View>
                                                            </View>
                                                        ))
                                                    ) : (
                                                        <Text style={{ color: Colors.textMuted, textAlign: 'center', padding: 16, fontSize: 13 }}>Hareket verisi bulunamadı.</Text>
                                                    )}
                                                </View>
                                            )}
                                        </View>
                            ...
                            <View style={[styles.emptyState, { marginTop: 30 }]}>
                                <Ionicons name="barbell-outline" size={64} color={Colors.textMuted} />
                                <Text style={styles.emptyText}>Henüz antrenman girişi yapmadın.</Text>
                                <Text style={[styles.emptyText, { fontSize: 13, marginTop: 4 }]}>Yukarıdaki butona tıklayarak başla!</Text>
                            </View>
                        )}
                    </>
                )}
